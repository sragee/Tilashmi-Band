  ...
  return (
    <Reveal delay={delay}>
      <div className="group relative overflow-hidden rounded-3xl glass">
        <div className="relative aspect-3/4 sm:aspect-4/5 overflow-hidden bg-surface-2">
          {member.photoUrl ? (
            <ResponsiveImage
              src={member.photoUrl}
              alt={member.name}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6">
            ...
            <h3 className="font-display text-xl sm:text-2xl text-white">{member.name}</h3>
          </div>
        </div>
        <div className="p-6">
          <p className="text-sm sm:text-base text-muted leading-relaxed line-clamp-3">{member.bio}</p>
