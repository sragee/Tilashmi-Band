        ...
        {messages.map((m) => (
          <div key={m.id} className={`rounded-2xl glass p-6 ${!m.read ? "border border-black/20" : ""}`}>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              ...
              <p className="text-xs text-black/40 shrink-0">
            ...
            <div className="mt-4 flex items-center gap-2">
              {!m.read && (
                <form action={markMessageRead}>
                  ...
                  <button
                    type="submit"
                    className="rounded-full border border-black/15 px-3.5 py-2 text-xs text-black/50 hover:text-black hover:border-black/30"
                  >
                    Mark as Read
                  </button>
                </form>
